...
//	Out:	なし
//----------------------------------------------
COrbitBul::COrbitBul(LPDIRECT3DDEVICE8 d3dDevice, LPDIRECT3DTEXTURE8 pTexture, D3DXMATRIX matBase)
			:CEffect(d3dDevice)
{
	CEffect::SetTexture(pTexture);				// テクスチャのセット
	C3DGraphicObj::SetMatrix(matBase);			// マトリクスのセット

	m_nParticleNum = 3;							// パーティクル数のセット
	m_fSize = 1.0f;								// サイズセット
	m_nMaxLife = 10;							// 最大生存カウンターのセット
	m_nLife = m_nMaxLife;						// 生存カウンターの初期化
	m_nInitR = 100;								// 初期色のセット
	m_nInitG = 255;
	m_nInitB = 100;
...
void COrbitBul::InitParticle()
{
	...
	for(int i=0; i<m_nParticleNum; i++)
	{
		...
		vecTmp.z += (float(rand()%3)/15.0f);		// 速度に差をつける
		vecTmp *= 0.5f;								// 全体的に速度を下げる
