...
//	Out:	なし
//----------------------------------------------
CExplosionB::CExplosionB(LPDIRECT3DDEVICE8 d3dDevice, LPDIRECT3DTEXTURE8 pTexture, D3DXMATRIX matBase)
			:CEffect(d3dDevice)
{
...
void CExplosionB::InitParticle()
{
	...
	for(int i=0; i<m_nParticleNum; i++)
	{
		...
		vecTmp.x = float(rand()%10-5)/10.0f;
		vecTmp.y = float(rand()%10-5)/10.0f;
		vecTmp.z = float(rand()%10-5)/10.0f;

		vecTmp.z += (float(rand()%3)/10.0f);		// 速度に差をつける
		vecTmp *= 0.3f;								// 全体的に速度を下げる
